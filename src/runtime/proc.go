...
func main() {
	...
	// 不需要p运行的线程是false, 例如sysmon 和 newm Handoff
	m0.doesPark = true
...
func schedule() {
	...
	// 如果当前g的m存在lockedg（lockOSThread或可能是系统调用中）
	if _g_.m.lockedg != 0 {
		// 解绑当前的m与p
		stoplockedm()
...
top:
	...
	// 如果gp存在绑定的m（不是当前的m），则解绑当前的p，并将p绑定到
	// lockedm，然后唤醒之前的m，随后进入阻塞状态（mstop)
	if gp.lockedm != 0 {
